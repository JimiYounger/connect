// src/app/(auth)/admin/analytics/video/[id]/page.tsx
'use client'
...
export default function VideoAnalyticsPage({ params }: { params: Promise<PageParams> }) {
  ...
  const fetchVideoAnalytics = useCallback(async (orgFilters: any = {}) => {
    ...
    try {
      // Fetch video overview with same timeframe as breakdown
      const overviewResponse = await fetch(`/api/analytics/video/${videoId}/overview?timeframe=30`)
