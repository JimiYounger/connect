...
// @ts-ignore: Works in Deno but not in Next.js build
...
// Check if we're in the Deno environment
...
if (IS_DENO) {
  ...
  console.log("🔄 Not in Deno environment - this is just a build step");
}

// Try to get environment variables with multiple possible keys
function getEnvVar(possibleKeys: string[]): string | null {
  if (!IS_DENO) return null; // Skip during Next.js build
...
// Conditional serve to prevent errors during Next.js build
if (IS_DENO) {
    ...
    try {
      ...
      try {
        ...
        return new Response(JSON.stringify({ 
          ...
          message: (bodyError as Error).message 
        }), {
          status: 400,
          headers: { "Content-Type": "application/json" },
        });
 ...
 */
async function processChunkBatch(chunks: any[], client: any) {
  const results = {
    success: 0,
    failed: 0,
  };
...
async function generateEmbeddings(texts: string[]): Promise<number[][]> {
  if (!IS_DENO) {
    console.warn("Cannot generate embeddings outside of Deno environment");
    return [];
  }
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      if (retries < MAX_RETRIES) {
        ...
        throw new Error(`Failed to generate embeddings after ${MAX_RETRIES} attempts: ${(error as Error).message}`);
      }
    }
  }
