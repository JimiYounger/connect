...
// Vector dimension for the new text-embedding-3-large model
...
Deno.serve(async (req)=>{
  try {
    console.log("🔄 Request received", {
      ...
      headers: Object.fromEntries([
        ...req.headers.entries()
          ...
          k,
          k.toLowerCase().includes("key") || k.toLowerCase().includes("auth") ? "***" : v
        ]))
    });
    ...
    const { data: chunks, error } = await client.from("document_chunks").select("id, content").eq("document_id", documentId).is("embedding", null);
    if (error) {
    ...
    for(let i = 0; i < chunks.length; i += BATCH_SIZE){
      ...
      try {
        ...
        results.failedChunkIds = [...results.failedChunkIds, ...batchResults.failedChunkIds];
        console.log(`✅ Batch processed: ${batchResults.success} succeeded, ${batchResults.failed} failed, ${batchResults.skipped} skipped`);
      } catch (batchError) {
        console.error("❌ Error processing batch:", batchError);
        
        // Add all chunks in this batch to the failed list
    ...
    } else if (results.failed + results.skipped === results.total) {
      // Mark as failed if all chunks failed or were skipped
      await client.from('documents').update({
 ...
 */ async function processChunkBatch(chunks, client) {
  ...
  if (validChunks.length === 0) {
    console.log("⚠️ No valid chunks to process in this batch");
    return results;
  }

  console.log(`⚙️ Generating embeddings for ${validChunks.length} chunks`);
  
  try {
    // Generate embeddings for all valid chunks in this batch
    const embeddings = await generateEmbeddings(validChunks.map(chunk => chunk.content));

    if (!embeddings || embeddings.length !== validChunks.length) {
      console.error("❌ Embedding generation failed or returned wrong count", {
        expected: validChunks.length,
      ...
      results.failed += validChunks.length;
      
      throw new Error("Failed to generate embeddings for batch");
    }

    // Update all chunks with their embeddings in parallel
    const updatePromises = validChunks.map((chunk, i) => {
      return {
        chunk,
    ...
    const settledResults = await Promise.allSettled(
      updatePromises.map(async ({ chunk, embedding }) => {
        console.log(`📝 Updating chunk ${chunk.id} with ${VECTOR_DIM}-dimensional vector`);
        
        const { error } = await client.from("document_chunks").update({ embedding }).eq("id", chunk.id);
    ...
    settledResults.forEach((result, index) => {
      const chunkId = validChunks[index].id;
      
      if (result.status === 'fulfilled') {
        console.log(`✅ Successfully updated chunk ${chunkId}`);
        results.success++;
      } else {
        console.error(`❌ Error updating chunk ${chunkId}:`, result.reason);
        results.failed++;
        results.failedChunkIds.push(chunkId);
      }
    });

  } catch (error) {
    console.error("❌ Batch processing error:", JSON.stringify({
      stage: "embedding_generation",
      error: error.message,
      stack: error.stack
    }));
    
    // If we haven't already marked chunks as failed (due to a more specific error)
    if (results.failedChunkIds.length === 0) {
    ...
    try {
      ...
      for (const embedding of embeddings) {
        if (embedding.length !== VECTOR_DIM) {
          throw new Error(`Vector length mismatch: expected ${VECTOR_DIM}, got ${embedding.length}`);
        }
      }
